          ...
          <div class="row">
            <div class="col-12">
              <div class="card-box table-responsive">
                ...
                <div class="col-md-3">
                  <a href="{{URL::to('admin/episodes/add_episode')}}" class="btn btn-success btn-md waves-effect waves-light m-b-20 mt-2 pull-right" data-toggle="tooltip" title="{{trans('words.add_episode')}}"><i class="fa fa-plus"></i> {{trans('words.add_episode')}}</a>
                </div>
              </div>
              ...
              <div class="row">   
                ...
                <div class="row">
                  @foreach($episodes_list as $i => $episodes)
                    <div class="col-lg-3 col-md-4 col-sm-4 col-xs-6" id="card_box_id_{{$episodes->id}}">

                        <!-- Simple card -->
                        <div class="card m-b-20">
                             <div class="checkbox checkbox-success wall_check">
                                <input type="checkbox" name="post_ids[]" id="checkbox<?php echo $i; ?>" value="<?php echo $episodes->id; ?>" class="post_ids">
                                <label for="checkbox<?php echo $i; ?>"></label>
                              </div>

                            @if(isset($episodes->video_image)) <img class="card-img-top thumb-lg img-fluid" src="{{URL::to('/'.$episodes->video_image)}}" alt="{{ stripslashes($episodes->video_title) }}"> @endif

                            <div class="card-body p-3">
                                <h4 class="card-title mb-1">{{ stripslashes($episodes->video_title) }}</h4>
								<p class="wall_sub_text mb-3">{{ stripslashes($episodes->series_name) }}</p>
                               <a href="{{ url('admin/episodes/duplicate_episode/'.$episodes->id) }}" class="btn btn-icon waves-effect waves-light btn-info m-r-5" data-toggle="tooltip" title="{{trans('words.duplicate')}}"> <i class="fa fa-copy"></i> </a>
                                <a href="{{ url('admin/episodes/edit_episode/'.$episodes->id) }}" class="btn btn-icon waves-effect waves-light btn-success m-r-5" data-toggle="tooltip" title="{{trans('words.edit')}}"> <i class="fa fa-edit"></i> </a>
                                <a href="#" class="btn btn-icon waves-effect waves-light btn-danger data_remove" data-toggle="tooltip" title="{{trans('words.remove')}}" data-id="{{$episodes->id}}"> <i class="fa fa-remove"></i> </a>
                                <a href="Javascript:void(0);" class="ml-2 fl-right mt-1" data-toggle="tooltip" title="@if($episodes->status==1){{ trans('words.active')}} @else {{trans('words.inactive')}} @endif"><input type="checkbox" name="category_on_off" id="category_on_off" value="1" data-plugin="switchery" data-color="#28a745" data-size="small" class="enable_disable"  data-id="{{$episodes->id}}" @if($episodes->status==1) {{ 'checked' }} @endif/></a>    
                            </div>
                        </div>
...
$(".enable_disable").on("change",function(e){      
      ...
      var action_name='episode_status';
...
<script type="text/javascript">
 ...
 $(".data_remove").click(function () {  
   ...
   var post_id = $(this).data("id");
   var action_name='episodes_delete';
...
$("#data_remove_selected").click(function () {  
  ...
  var action_name='episodes_delete_selected';
